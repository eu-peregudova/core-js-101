...
function isLeapYear(date) {
  const check = new Date(`${date.getFullYear()}-02-29`);
...
function timeSpanToString(startDate, endDate) {
  ...
  mils = mils.toString();
  if (mils.length < 3) {
    mils = '0'.repeat(3 - mils.length) + mils;
  }
  ...
  mins = mins.toString();
  if (mins.length < 2) {
    mins = '0'.repeat(2 - mins.length) + mins;
  }
...
function angleBetweenClockHands(date) {
  ...
  const hour = dateNew.getUTCHours() % 12;
  ...
  return Math.sqrt(degreesToRadians(angle) ** 2);
}
